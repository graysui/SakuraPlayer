...
- `install-latest.sh` 是新手推荐的远程引导器：通过 `releases/latest` 重定向解析规范 `vX.Y.Z`，按固定资产名称下载归档，在临时目录解压后把固定发布文件复制到执行命令时的当前目录，再调用该持久目录内的 `install.sh`。新归档必须包含自身副本；为兼容已发布的旧归档，缺少 `install-latest.sh` 时仍可安装，只要 Compose、环境模板、版本文件、`install.sh`、部署说明和许可证文件齐全。它不要求用户手动下载、解压或执行 SHA-256；首次安装可选择合法 IPv4 发布地址和 API 端口并写入 `.env`。现有安装只把单一 Docker Hub/GHCR 官方完整 SemVer 镜像升级到当前 Release 并保留 registry；相同版本幂等，降级、自定义、本地、digest、`latest`、缺失或重复镜像配置在发布文件覆盖前拒绝。`.env` 其他行、Compose override、`secrets/` 和 `data/` 保持不变。新归档的 Compose 使用当前目录 `data/` bind mounts；旧归档的 `sakuraplayer_*` named volume 在启动前复制到对应目录且不删除原卷。已有数据库会先同步当前 PostgreSQL secret 对应的角色密码。非法版本、下载失败和归档布局异常必须在 Compose 启动前失败并清理临时目录；运行配置与业务数据不得依赖临时目录。
