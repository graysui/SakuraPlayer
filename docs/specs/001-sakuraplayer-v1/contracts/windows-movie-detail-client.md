...
本契约固定 Windows 影片详情对既有 `/api/v1/movies/{movie_id}`、影片收藏、影片级重新刮削和认证目录图片的消费、路由、来源选择、失败恢复与桌面布局。后端聚合顺序、来源可用性、影片进度和收藏真相继续以 `rest-api.openapi.yaml`、Catalog/Discovery 端口和 TASK-011/TASK-103/TASK-105/TASK-111 变更规格为准。
...
- 重新刮削只调用 `POST admin/movies/{movie_id}/metadata-rescrape`，请求体为空；响应严格读取 UUID `job_id`、`queued/running`、布尔 `created`。客户端不得提交番号、优先级、provider 或阶段。
...
- 重新刮削与收藏使用独立在途状态。连续点击不得并发请求；成功只显示“已加入最高优先级刮削队列”或“当前番号正在刮削”，失败保留详情、收藏和来源选择并显示中文错误。认证代次或 MovieId 变化必须清除旧反馈并忽略迟到结果。
...
- `release_date` 优先作为影片日期，缺失时回退摘要 `publish_date`；中日标题只有在不同时才单独显示。简介区只显示 `description` 中文译文，禁止显示 `description_original`；缺少译文时显示“暂无中文简介”。缺失厂商、系列、导演、评分、演员、标签或剧照分别使用稳定空状态，不阻断来源列表。
...
- 资料头之后的连续滚动区块固定按“来源、简介、剧照”排列，宽窄布局顺序一致；不得为调整顺序新增嵌套卡片或独立滚动区。
- 资料头操作区新增固定尺寸刷新图标加“重新刮削”命令；core-ready 和受限详情均可使用，在途时禁用且不改变标题、收藏或播放按钮几何。

## 7. 验证

- API/DTO：完整/缺失富化、共享摘要/演员、集合上限与重复、日期、标签、availability、两个大小、认证图片路径、204 收藏、重新刮削严格响应与未知协议值。
- Controller：MovieId/generation、迟到响应、404/普通失败、重试、收藏与重新刮削的在途/成功/失败、显式来源选择、rejected 禁用、选择失效和认证清理。
- Widget/Route：四类入口到详情、详情到演员、非法 UUID、返回、宽窄布局、长文本、仅中文简介、缺图/字段/集合、剧照失败、叠加标签、六状态、大小文案、进度、收藏、重新刮削、无历史/列表入口和 source_id-only 输出。
