...
**当前阶段**: TASK-310 completed：HarmonyOS AVPlayer 播放器（原画/HLS 固定 UA 与 seek）。下一步 TASK-311（播放器字幕/轨道/进度心跳）。
