...
- TASK-323 已修复首次 host/port 输入写回安装目录 `.env`、新 Compose 使用 `data/` bind mount、旧 named volume 复制迁移，以及旧数据库角色密码与宿主 PostgreSQL secret 不一致导致的迁移失败；远程引导器 Docker 依赖改为按需调用，并修复 Bash SQL 输出与多目录创建语法。Linux 安装器回归为 `18 passed`，Compose 结构测试已改为 bind mount 断言，待 CI/飞牛 NAS 实测。
...
- **最终门禁状态**: TASK-323 未运行完整后端 Compose；安装器自动测试已通过，Compose 结构测试因本机 WSL 未接入 Docker 未执行，待 CI/用户实测实际 `.env` 写回、data 目录、数据库迁移和服务健康。
