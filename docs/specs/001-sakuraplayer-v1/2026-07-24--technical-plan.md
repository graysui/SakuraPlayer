...
**决策**: 已认证接口创建 12 小时播放会话，签名绑定管理员 session epoch、平台 UA、缓存、媒体和模式。流入口校验后解析上游地址并返回 `302 no-store`。默认原画；取链失败或用户选择兼容播放时解析最高码率 HLS。
...
**决策**: 领域事务同时写版本化事件。WebSocket 推送事件，客户端按 `event_id` 去重；首次连接、重连或游标过旧时获取 REST 快照。

**后果**: 不需要保证 WebSocket 逐条不丢；事件表需按保留策略清理，客户端不能把事件当最终状态。
...
- 播放器只有原画/兼容播放、字幕、音轨、倍速、全屏和标准进度控制。
