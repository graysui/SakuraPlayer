...
ac-mapping: [AC-001, AC-002, AC-004, AC-010, AC-011, AC-012]
imp-requirements: [REQ-001, REQ-003]
...
**功能描述**: 实现首次引导唯一管理员、Argon2id 密码、短期访问令牌、可撤销刷新会话和退出登录清理信号。

**规格映射**: AC-001、AC-002、AC-004、AC-010 至 AC-012

## 验收条件

- [ ] 首次部署可创建唯一管理员，重复创建和注册路径均被拒绝；对应 AC-001。
...
- [ ] Argon2id、JWT 访问/刷新有效期和 session epoch 已按技术计划冻结。
...
- 验证同一管理员第二次 bootstrap、无 token、过期 refresh 的错误码。
...
- 并发 bootstrap、重复 refresh、刷新令牌重放、密码边界长度。
