...
ac-mapping: [AC-037, AC-038, AC-039, AC-040, AC-041, AC-042, AC-043]
imp-requirements: [REQ-008]
...
**功能描述**: 实现 PostgreSQL 元数据队列、固定三子进程 supervisor、600 秒进程组硬终止、五级优先级和仅管理员手动重试。

**规格映射**: AC-037 至 AC-043
...
- `failed` 行不可复位；retry 新建 parent_job_id 指向旧任务。
...
- 验证失败只允许显式 manual retry 新建 attempt，普通 worker 不重排。
