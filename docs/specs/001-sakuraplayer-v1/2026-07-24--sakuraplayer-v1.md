...
- **AC-077 `[IMP]`**: 首版提供可分页查看的影片和女优单一收藏集合，不提供多个自定义播放列表。
...
- **AC-119 `[IMP]`**: 客户端设置页管理 115、JavDB、AI、缓存期限、同步状态和连接测试；非敏感现值可回显，密码、Cookie 与 API key 只返回是否已配置。
...
- **AC-133 `[IMP]`**: 首次创建唯一管理员必须同时提供由 Docker Secret 或环境变量注入的一次性初始化口令；服务端常量时间校验，管理员创建后永久拒绝再次 bootstrap，口令不得入库、入日志或返回客户端。
