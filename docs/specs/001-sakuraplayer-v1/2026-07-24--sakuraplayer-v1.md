...
- **AC-046 `[IMP]`**: JavDB 用户名和密码是可选的加密配置；未配置时跳过需要登录的 TOP250，不影响其他功能。
...
- **AC-054 `[IMP]`**: 后端支持 OpenAI 兼容接口，可配置 `base_url`、`api_key`、`model` 和超时。
- **AC-055 `[IMP]`**: AI 异步翻译影片标题和简介；演员简介仅在映射表缺少中文内容时翻译。
- **AC-056 `[IMP]`**: 番号、演员姓名、厂商、系列和标签不得由 AI 自由改写。
- **AC-057 `[IMP]`**: 原文、译文、来源内容摘要、模型和提示版本必须持久化；来源未变化时不得重复付费翻译。
...
- **AC-063 `[IMP]`**: 媒体库使用一个去重影片网格，六个 AVdb 分类作为可组合筛选条件。
- **AC-064 `[IMP]`**: 默认按 AVdb 发布日期从新到旧排序，并支持字幕、破解、4K、有码、来源、可播放状态和资源大小筛选。
- **AC-065 `[IMP]`**: 全局搜索支持番号、影片标题、演员姓名和别名，结果按影片与女优分组。
- **AC-066 `[IMP]`**: 搜索命中尚未刮削的 AVdb 番号时显示“正在补全资料”，创建最高优先级元数据任务，完成后刷新正式结果。
- **AC-067 `[IMP]`**: 只有核心元数据成功的影片可以显示正式影片卡片和详情页。
- **AC-068 `[IMP]`**: 影片卡片和详情页播放按钮显示影片级播放进度或已看完状态。

### REQ-014 排行榜

- **AC-069 `[IMP]`**: 排行榜使用 JavDB 本地快照，不在页面打开时实时抓取。
- **AC-070 `[IMP]`**: 页面支持日榜、周榜、月榜、TOP250，以及榜单适用的年度筛选。
- **AC-071 `[IMP]`**: 榜单只展示存在 AVdb 资源且核心元数据已完成的影片。
- **AC-072 `[IMP]`**: 榜单命中“有 AVdb 资源但元数据未完成”的影片时，创建高优先级刮削任务。
- **AC-073 `[IMP]`**: 榜单同步失败时保留最近一次成功快照，不清空现有榜单。

### REQ-015 影片详情与女优详情

- **AC-074 `[IMP]`**: 影片详情展示封面、中日文标题、番号、日期、厂商、系列、导演、演员、标签、评分、简介、剧照、观看进度、收藏和多来源资源。
- **AC-075 `[IMP]`**: 女优列表支持姓名和别名搜索。
- **AC-076 `[IMP]`**: 女优详情展示头像、中日文名、别名、简介、写真图库、关联影片网格和收藏状态。
- **AC-077 `[IMP]`**: 首版提供影片和女优的单一收藏集合，不提供多个自定义播放列表。
...
- **AC-115 `[IMP]`**: 后端通过 WebSocket 推送离线、缓存、刮削和凭据状态变化。
- **AC-116 `[IMP]`**: 客户端重连后必须通过 REST 快照恢复任务状态，不依赖遗漏的事件。
- **AC-117 `[IMP]`**: 应用运行或处于系统后台时显示离线完成通知；完全退出后不常驻，下次启动补拉结果。
- **AC-118 `[IMP]`**: 顶部缓存入口显示排队、运行和就绪数量角标。

### REQ-022 管理员设置与诊断

- **AC-119 `[IMP]`**: 客户端设置页管理 115、JavDB、AI、缓存期限、同步状态和连接测试。
- **AC-120 `[IMP]`**: 主密钥等启动级机密只能由 Docker Secret 或环境变量提供，不得通过客户端修改。
- **AC-121 `[IMP]`**: 诊断页显示脱敏后的任务阶段、稳定错误码、耗时、尝试次数和连接测试结果。
- **AC-122 `[IMP]`**: 管理员可查看并手动重试失败元数据任务、取消排队或运行中的离线任务、清理就绪缓存。
...
- **AC-127 `[IMP]`**: API、工作进程和 PostgreSQL 提供健康检查；进程重启后任务状态可对账恢复。

### REQ-024 测试与外部验收

- **AC-128 `[IMP]`**: 默认自动测试不得访问真实 115、JavDB 写操作或真实 AI 付费接口，必须使用替身和固定样本。
- **AC-129 `[IMP]`**: AVdb 解密、幂等导入、番号合并、分类标签、元数据超时、任务优先级、缓存状态机、安全删除、签名校验、播放进度和字幕生命周期都有自动测试。
...
Windows v1 只有在 AC-001 至 AC-130 中适用于 Windows/后端的 `[IMP]` 项通过自动验证，且 AC-130 真实 115 检查全部通过后才能发布。HarmonyOS 工作只有在 Windows v1 门禁完成且 AC-131 的真实设备探针全部通过后才能开始。
