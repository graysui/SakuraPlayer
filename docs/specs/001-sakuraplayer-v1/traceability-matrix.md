...
- AC-084 至 AC-091、AC-117 的 Windows play-request DTO/gateway、服务器 deadline + 单调倒计时、等待导航锁、取消/窗口关闭和即时通知适配器由 [TASK-209 Windows 播放请求、等待与通知边界](changes/2026-07-31--task-209-playback-wait-notification-boundaries.md)、[Windows 播放请求客户端契约](contracts/windows-play-request-client.md) 与 [ADR-004](../adr/ADR-004-windows-cache-notifications.md) 冻结；TASK-209 在其前置契约提交后实施，真实通知/安装包验收继续归 TASK-212/213。
