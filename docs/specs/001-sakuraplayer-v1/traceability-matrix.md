...
- AC-054 至 AC-058 的加密配置快照、单字段 JSON、owner 作用域幂等键和付费派发事实由 [TASK-010 翻译协议与付费幂等边界](changes/2026-07-26--task-010-translation-safety-boundaries.md) 冻结并由 TASK-010 实现；prompt v2、硅基流动 Qwen3.5 非思考 profile、动态输出上限、安全诊断和旧 v1 事实隔离由 [TASK-225 硅基流动 Qwen 翻译协议兼容](changes/2026-08-02--siliconflow-qwen-translation-compatibility.md) 冻结并由 TASK-225 实现；仅标题/影片简介、protected 本地占位、v3 输出瘦身和旧 v1/v2 事实隔离由 [TASK-325 AI 配置恢复、翻译瘦身与 Docker 原地升级](changes/2026-08-06--task-325-ai-settings-translation-docker-upgrade.md) 冻结并由 TASK-325 实现；REQ-CHG-075 的 `base_url` 不含 `/v1` 尾段语义由 [AI base_url 允许 `/v1` 尾段](changes/2026-08-06--ai-base-url-v1-suffix.md)（REQ-CHG-323/324）修订；OpenCode Go deepseek-v4 思考关闭与占位符保护移除由 [OpenCode Go deepseek-v4 翻译适配与占位符保护移除](changes/2026-08-06--opencode-translation-adaptation.md)（REQ-CHG-325/327）冻结。
- AC-040/041/055/057/074/122 的详情中文-only 简介、影片级 priority 10 完整重新刮削、活动 attempt 复用和旧翻译事实保护由 [TASK-227 影片详情中文简介与重新刮削](changes/2026-08-03--movie-detail-chinese-description-rescrape.md) 冻结并由 TASK-227 实现；其中"Windows 不显示 `description_original`、缺失译文显示'暂无中文简介'"条款由 [详情页翻译缺失时回退显示原文简介](changes/2026-08-06--translation-fallback-to-original-description.md)（REQ-CHG-328）修订。
...
- AC-063、AC-064、AC-067、AC-068、AC-077 的 Windows DTO 所有权、认证封面、固定桌面几何、筛选 generation、游标恢复和追加失败语义由 [TASK-204 Windows 媒体库客户端边界](changes/2026-07-30--task-204-library-client-boundaries.md) 与 [Windows 媒体库客户端契约](contracts/windows-library-client.md) 冻结；TASK-204 已完成客户端实现，自动证据位于 `library_controller_test.dart`、`library_page_test.dart`、`search_controller_test.dart` 和 `app_bootstrap_test.dart`；其中 AC-064 的"来源最新发布日期"排序键由 [媒体库排序改用影片发行日期](changes/2026-08-06--library-sort-by-release-date.md)（REQ-CHG-329）修订为影片发行日期。
...
TASK-321 的 AC-134/143/144/147/148 映射受
[Linux Docker 持久化安装与网络选择](changes/2026-08-04--task-321-persistent-docker-install.md)、
[运行配置契约](contracts/runtime-configuration.md) 和 [GitHub 自动发布契约](contracts/github-release.md)
...
TASK-323 的 AC-148/149 映射受
[Linux Docker 数据目录与配置兼容](changes/2026-08-04--task-323-docker-data-and-config-compatibility.md)、
[运行配置契约](contracts/runtime-configuration.md) 和 [GitHub 自动发布契约](contracts/github-release.md)
约束；它新增 AC-149，当前有效任务总数增至 76。

TASK-324 的 AC-119/150 映射受
