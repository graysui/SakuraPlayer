...
| 运行修复 | 1 | 0 | 0 | 对应缺陷已在真实运行中复现 | [任务列表](2026-08-01--runtime-fixes--tasks.md) |

**合计**: 54 个实现任务、5 个 E2E 任务、4 个清理任务，共 63 个任务。每个工作流的实现任务数不超过 15。
...
| 工作流规模 | PASS；五个工作流分别 15/12/15/11/1 个实现任务，满足每个工作流不超过 15 个实现任务的约束 |
