...
**合计**: 64 个实现任务、5 个 E2E 任务、4 个清理任务，共 73 个任务。每个工作流的实现任务数不超过 15。
...
| 门禁 | 结果/要求 |
|---|---|
| 规格分类 | 137 `[IMP]`、4 `[SEF]`、3 `[EXT]` 已保留 |
| 工作流规模 | PASS；五个既有工作流分别 15/12/15/11/8 个实现任务，另有 TASK-316/317/318 三个独立发布任务；各工作流均不超过 15 个实现任务 |
